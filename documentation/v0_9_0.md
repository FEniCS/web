...
This release marks a milestone in DOLFINx development, as several new features have been added, see summary below:
...
In DOLFINx, we now support assembly of 0th, 1st and 2nd order tensors consisting of meshes from different sub-meshes. One can now create function space over sub-meshes, and assemble with coefficients, test-functions or trial-functions from the parent mesh (and vici versa).  
The feature currently works for mixed assembly of co-dimension 0 (sub-set of cells) and co-dimension 1 (sub-set of facets).
...
Main contributor: [Jørgen S. Dokken](https://github.com/jorgensd/)  
To unify the C++ and Python interface with UFL and Basix, the user can now create data-independent variational forms for Python (similar to what is done for C++ problems), and then in turn attached data to the compiled form. This means that the same code can be used for both C++ and Python programs.  
...
**Main contributors:** [Jack Hale](https://github.com/jhale/) and [Min Ragan-Kelley](https://github.com/minrk/)

There is now support in DOLFINx for pure windows builds (without PETSc). You can install DOLFINx with conda on Windows, see: [https://github.com/FEniCS/dolfinx/?tab=readme-ov-file\#conda](https://github.com/FEniCS/dolfinx/?tab=readme-ov-file#conda) for details.
...
The `dolfinx.mesh.Topology` and `dolfinx.mesh.Geometry` classes are now fully wrapped in Python and properly documented, [DOLFINx \#3406](https://github.com/FEniCS/dolfinx/pull/3406), [DOLFINx \#3403](https://github.com/FEniCS/dolfinx/pull/3403), [DOLFINx \#3400](https://github.com/FEniCS/dolfinx/pull/3400)
...
Previously only up to 3x3 tensors and length 3 vectors would be outputted. [DOLFINx \#3227](https://github.com/FEniCS/dolfinx/pull/3227) resolves this.

#### Interpolation matrix with built in matrices {#imatrix}
